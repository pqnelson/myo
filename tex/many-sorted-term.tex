...
of variables $\Gamma$. The basic idea is:
\begin{enumerate}\setcounter{enumi}{-1}
\item Given: a string $x$ and a list of variables $\Gamma$. Set
  $n\gets 0$.
\item If $x\notin\Gamma$, then just use $x$ and terminate the
  algorithm. Otherwise go to the next step.
\item Append a suffix $n$ to the string $x$ to obtain $x_{n}$ and
  treat it like a  possible variable.
\item Check if $x_{n}\notin\Gamma$---in which case, return $x_{n}$ as a
  variable and terminate the algorithm. Otherwise increment $n\gets n+1$
  and go to the previous step.
\end{enumerate}
Since the list of variables is a finite list, we must eventually
