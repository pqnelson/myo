...
\bigbreak
\item \checkbox{0} $\vdash(\forall x\ldotp(C\implies A[x]))\iff(C\implies\forall x\ldotp A[x])$%f
...
\item \checkbox{0} $\vdash(\exists x\ldotp(C\implies A[x]))\implies(C\implies\exists x\ldotp A[x])$%h
...
\nwalsodefined{\\{NW1NjfzY-uZfts-2}\\{NW1NjfzY-uZfts-3}\\{NW1NjfzY-uZfts-4}\\{NW1NjfzY-uZfts-5}\\{NW1NjfzY-uZfts-6}\\{NW1NjfzY-uZfts-7}\\{NW1NjfzY-uZfts-8}\\{NW1NjfzY-uZfts-9}\\{NW1NjfzY-uZfts-A}\\{NW1NjfzY-uZfts-B}}\nwused{\\{NW1NjfzY-4dPl0p-1}}\nwendcode{}\nwbegindocs{84}\nwdocspar
...
fun all_conj_distrib_test () =
  ...
  in
    if not(Formula.eq expected actual)
    then "Expecting: "^(Formula.serialize expected)^"\\nActual: "^
...
fun ex_imp_test () =
  let
    ...
    val actual = Thm.concl (ex_imp x C A);
  in
    if not(Formula.eq expected actual)
    then "Expecting: "^(Formula.serialize expected)^"\\nActual: "^
         (Formula.serialize actual)^"\\n"
