...
\item If $t$ is a function $f(t_{1},\dots,t_{n})$, then we want to test 
  ...
  would lead to a cyclic environment.

  If there is a possible extension $x\sim t_{k}$, then raise a
  \textbf{Fail} exception.

  Otherwise, return \textbf{false} to indicate \emph{we can extend the
  environment while maintaining the cycle-free invariant}.
\end{enumerate}
This algorithm is the famously cryptic ``occurs check'' for
unification. It is commonly buggy, see Norvig~\cite{norvig1991correcting}
for a review.
\end{node}
