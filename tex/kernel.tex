...
experiment\footnote{\url{https://codeberg.org/pqnelson/ifol}} for single-sorted Intuitionistic First-Order Logic, I used
...
\begin{node}
The basic judgement for our kernel is the theorem, which takes the
form $\Gamma\vdash\phi$. We should read this as ``Under the
[finite list of] assumptions $\Gamma$, we can prove $\phi$''.

We start off with an infixed operator approximating the turnstile. But
this is an opaque type: the user cannot construct theorems
willy-nilly, but only by using the inference rules we provide. How
does the user know what has been proven? We provide destructuring
functions {\Tt{}concl\nwendquote} to access $\phi$ and {\Tt{}hyps\nwendquote} to access $\Gamma$
from the theorem $\Gamma\vdash\phi$.
\end{node}
...
an additional $\bigOh(n\log n)$ runtime to each invocation.
\end{node}
...
\end{equation}
This is the only axiom schema in natural deduction: if $\phi$ is among
the assumptions made, then we may infer $\phi$ is proven in that context.

Discharging a local assumption (transforming it into a premise of a
logical implication) is the same as ``implication introduction'' according to
Kleene's \textit{Introduction to Mathematical Logic} (\S11, p.44):
\begin{equation}
\vcenter{\infer[\texttt{disch}(\phi)]{\Gamma\setminus\{\phi\}\vdash\phi\implies\psi}{\Gamma\vdash\psi}}
\end{equation}
Note that we do not require $\phi\in\Gamma$ in the discharge
rule. What happens if $\phi\notin\Gamma$? How could this be justified?
Logicians call such a situation ``weakening'', which describes adding
an un-necessary premise $\phi$.

We have the ``dual'' to discharging a local assumption, we transform a
premise into a local assumption via ``undischarging'':
\begin{equation}
\vcenter{\infer[\texttt{undisch}]{\phi,\Gamma\vdash\psi}{\Gamma\vdash\phi\implies\psi}}
\end{equation}
Unlike discharging, an error will be raised if the user attempts to
undischarge a theorem which does not have a conclusion of the form
$\phi\implies\psi$. 

The \textit{modus ponens} is really the same thing as ``implication elimination''
(again, by Kleene, \textit{ibid}):
\begin{equation}
\vcenter{\infer[\texttt{modus\_ponens}]{\Gamma_{1}\cup\Gamma_{2}\vdash\psi}{\Gamma_{1}\vdash\phi\implies\psi,&\Gamma_{2}\vdash\phi}}
\end{equation}
The other inference rules for connectives are more straightforward
...
\end{equation}
Similarly, if we have already proven $\Gamma\vdash\phi\land\psi$, then
we can ``break apart'' the conjunction to get each conjunct.
\begin{equation}
...
fun and_elim_r (Gamma |- phi) =
  ...
  then raise Fail "and_elim_r: expects conjunction"
    ...
    let val (A,B) = Formula.dest_and phi
    in Gamma |- B end;
\nwindexdefn{\nwixident{Thm.and{\_}intro}}{Thm.and:unintro}{NW4ICsJQ-2zKkPU-3}\nwindexdefn{\nwixident{Thm.and{\_}elim{\_}l}}{Thm.and:unelim:unl}{NW4ICsJQ-2zKkPU-3}\nwindexdefn{\nwixident{Thm.and{\_}elim{\_}r}}{Thm.and:unelim:unr}{NW4ICsJQ-2zKkPU-3}\eatline
...
\begin{node}
Disjunction introduction is straightforward: we can always assume ``or
something else'' holds. More precisely,
\begin{equation}
...
Note that there are other variations of $\lor$-elimination which are
logically equivalent to this inference rule. Most texts use the same
inference rule for $\lor$-elimination as found in Classical natural
deduction,
\begin{equation}
\vcenter{\infer{\Gamma\vdash C}{\Gamma\vdash A\lor B, & \Gamma,A\vdash C & \Gamma,B\vdash C}}
\end{equation}
Then discharging $A$ (resp., $B$) in the premises, including $A\lor B\in\Gamma$
and discharging $A\lor B$ from the conclusion, gives us our
$\lor$-elimination rule.
\end{node}
...
\begin{node}
Negation-related rules follow from ``unfolding'' (and ``folding'') the abbreviation $\neg A\equiv A\implies\bot$
as inference rules.
\begin{equation}
...
We have the inference rule for generalization: given any variable $x$
which does not appear [freely] in the local assumptions $\Gamma$, we
can infer $\forall x\ldotp\phi$ even if $x$ does not appear in $\phi$
(but this is a rather boring situation):
\begin{equation*}
\vcenter{\infer[\forall\mbox{-intro}(x)]{\Gamma\vdash\forall x\ldotp\phi}{\Gamma\vdash\phi,&x\notin FV(\Gamma)}}
\end{equation*}
In particular, this means that we cannot infer from $P[y]\vdash P[y]$ 
the claim $P[y]\vdash\forall y\ldotp P[y]$ unless $y\notin FV(P[y])$
(in which case $\forall y\ldotp P[y]$ is vacuous).
\end{node}
...
\end{equation}
This is just instantiating a universally quantified variable $x$ with
a term $t$. The only possible source of problems we could encounter is
if $t$ and $x$ have different sorts, which should raise a failure
exception.

A more subtle problem occurs when $t$ has a free variable which would
be captured by a quantifier upon substituting $t$ for $x$ in $P[x]$. 
For example, if $P[x]:=\forall y\ldotp x=y$ and $t=f(y)$, then
$P[f(y)]=\forall y\ldotp f(y)=y$ would result in capturing $y$ in $t=f(y)$.
This \emph{should} be an error, which will be raised in an exception.
\end{node}
...
elimination---$t$ is free for $x$ in $\phi[-]$:
\begin{equation}
...
be flagged as problematic. Why?

Well, this situation should never occur because $\phi[x]:=\forall x\ldotp(x=x)$
has $\phi[t]=\forall x\ldotp(x=x)$ which is syntactically different
from $(\forall x\ldotp t=x)$. So an error should be raised, catching
...
Also we should raise an exception whenever $t$ and $x$ have different
sorts.
\end{node}
...
Existential elimination takes the form borrowed from the Hilbert
...
\end{equation*}
Natural deduction usually takes a different variant for
$\exists$-elim, but we will show that it can be derived from our
inference rules (\S\ref{node:derived:existential-elimination}).
\end{node}
...
\nwalsodefined{\\{NW4ICsJQ-hYCIS-2}\\{NW4ICsJQ-hYCIS-3}\\{NW4ICsJQ-hYCIS-4}\\{NW4ICsJQ-hYCIS-5}\\{NW4ICsJQ-hYCIS-6}\\{NW4ICsJQ-hYCIS-7}\\{NW4ICsJQ-hYCIS-8}\\{NW4ICsJQ-hYCIS-9}\\{NW4ICsJQ-hYCIS-A}\\{NW4ICsJQ-hYCIS-B}\\{NW4ICsJQ-hYCIS-C}\\{NW4ICsJQ-hYCIS-D}\\{NW4ICsJQ-hYCIS-E}}\nwused{\\{NW4ICsJQ-2cY0B1-1}}\nwidentuses{\\{{\nwixident{Term.t}}{Term.t}}}\nwindexuse{\nwixident{Term.t}}{Term.t}{NW4ICsJQ-hYCIS-1}\nwendcode{}\nwbegindocs{48}\nwdocspar
...
\begin{node}[Decidable predicates]
We say a predicate $P[x]$ is \define{Decidable} if there exists a
recursive function $f$ such that $\vdash\forall x\ldotp f(x)=0\iff P[x]$.
In this situation, we can infer $\forall x\ldotp P[x]\lor\neg P[x]$.
This only matters for Intuitionistic logic (i.e., our situation).

In $FS_{0}$, we have a weaker notion of primitive recursive functions,
which are any function-sorted term constructed using the
axioms. Primitive recursive functions suffice for our purposes in
determining if a predicate is decidable or not.
This means we have the following additional axiom:
\begin{equation}
\vdash(\exists f:Fun\ldotp\forall x:Ind\ldotp f(x)=0\iff P[x])\implies\forall x:Ind\ldotp P[x]\lor\neg P[x].
\end{equation}
\end{node}

\nwenddocs{}\nwbegincode{101}\sublabel{NW4ICsJQ-hYCIS-E}\nwmargintag{{\nwtagstyle{}\subpageref{NW4ICsJQ-hYCIS-E}}}\moddef{Specification for $FS_{0}$ axioms~{\nwtagstyle{}\subpageref{NW4ICsJQ-hYCIS-1}}}\plusendmoddef\nwstartdeflinemarkup\nwusesondefline{\\{NW4ICsJQ-2cY0B1-1}}\nwprevnextdefs{NW4ICsJQ-hYCIS-D}{\relax}\nwenddeflinemarkup
val axiom_dec : (Term.t -> Formula.t) -> t;
\nwused{\\{NW4ICsJQ-2cY0B1-1}}\nwidentuses{\\{{\nwixident{Formula.t}}{Formula.t}}\\{{\nwixident{Term.t}}{Term.t}}}\nwindexuse{\nwixident{Formula.t}}{Formula.t}{NW4ICsJQ-hYCIS-E}\nwindexuse{\nwixident{Term.t}}{Term.t}{NW4ICsJQ-hYCIS-E}\nwendcode{}\nwbegindocs{102}\nwdocspar

\nwenddocs{}\nwbegincode{103}\sublabel{NW4ICsJQ-2aDdLu-E}\nwmargintag{{\nwtagstyle{}\subpageref{NW4ICsJQ-2aDdLu-E}}}\moddef{Axioms for $FS_{0}$~{\nwtagstyle{}\subpageref{NW4ICsJQ-2aDdLu-1}}}\plusendmoddef\nwstartdeflinemarkup\nwusesondefline{\\{NW4ICsJQ-YIPBT-1}}\nwprevnextdefs{NW4ICsJQ-2aDdLu-D}{\relax}\nwenddeflinemarkup
(* axiom_dec : (Term.t -> Formula.t) -> \nwlinkedidentc{Thm.t}{NW4ICsJQ-YIPBT-1} *)
fun axiom_dec P =
  let
...
\nwindexdefn{\nwixident{Thm.axiom{\_}dec}}{Thm.axiom:undec}{NW4ICsJQ-2aDdLu-E}\eatline
\nwused{\\{NW4ICsJQ-YIPBT-1}}\nwidentdefs{\\{{\nwixident{Thm.axiom{\_}dec}}{Thm.axiom:undec}}}\nwidentuses{\\{{\nwixident{Formula.bv}}{Formula.bv}}\\{{\nwixident{Formula.fv}}{Formula.fv}}\\{{\nwixident{Formula.mk{\_}exists}}{Formula.mk:unexists}}\\{{\nwixident{Formula.mk{\_}forall}}{Formula.mk:unforall}}\\{{\nwixident{Formula.mk{\_}iff}}{Formula.mk:uniff}}\\{{\nwixident{Formula.mk{\_}imp}}{Formula.mk:unimp}}\\{{\nwixident{Formula.mk{\_}not}}{Formula.mk:unnot}}\\{{\nwixident{Formula.mk{\_}or}}{Formula.mk:unor}}\\{{\nwixident{Formula.t}}{Formula.t}}\\{{\nwixident{FS0.app}}{FS0.app}}\\{{\nwixident{FS0.equal}}{FS0.equal}}\\{{\nwixident{FS0.zero}}{FS0.zero}}\\{{\nwixident{Sort.FUN}}{Sort.FUN}}\\{{\nwixident{Sort.IND}}{Sort.IND}}\\{{\nwixident{Term.fresh{\_}var}}{Term.fresh:unvar}}\\{{\nwixident{Term.t}}{Term.t}}\\{{\nwixident{Thm.t}}{Thm.t}}}\nwindexuse{\nwixident{Formula.bv}}{Formula.bv}{NW4ICsJQ-2aDdLu-E}\nwindexuse{\nwixident{Formula.fv}}{Formula.fv}{NW4ICsJQ-2aDdLu-E}\nwindexuse{\nwixident{Formula.mk{\_}exists}}{Formula.mk:unexists}{NW4ICsJQ-2aDdLu-E}\nwindexuse{\nwixident{Formula.mk{\_}forall}}{Formula.mk:unforall}{NW4ICsJQ-2aDdLu-E}\nwindexuse{\nwixident{Formula.mk{\_}iff}}{Formula.mk:uniff}{NW4ICsJQ-2aDdLu-E}\nwindexuse{\nwixident{Formula.mk{\_}imp}}{Formula.mk:unimp}{NW4ICsJQ-2aDdLu-E}\nwindexuse{\nwixident{Formula.mk{\_}not}}{Formula.mk:unnot}{NW4ICsJQ-2aDdLu-E}\nwindexuse{\nwixident{Formula.mk{\_}or}}{Formula.mk:unor}{NW4ICsJQ-2aDdLu-E}\nwindexuse{\nwixident{Formula.t}}{Formula.t}{NW4ICsJQ-2aDdLu-E}\nwindexuse{\nwixident{FS0.app}}{FS0.app}{NW4ICsJQ-2aDdLu-E}\nwindexuse{\nwixident{FS0.equal}}{FS0.equal}{NW4ICsJQ-2aDdLu-E}\nwindexuse{\nwixident{FS0.zero}}{FS0.zero}{NW4ICsJQ-2aDdLu-E}\nwindexuse{\nwixident{Sort.FUN}}{Sort.FUN}{NW4ICsJQ-2aDdLu-E}\nwindexuse{\nwixident{Sort.IND}}{Sort.IND}{NW4ICsJQ-2aDdLu-E}\nwindexuse{\nwixident{Term.fresh{\_}var}}{Term.fresh:unvar}{NW4ICsJQ-2aDdLu-E}\nwindexuse{\nwixident{Term.t}}{Term.t}{NW4ICsJQ-2aDdLu-E}\nwindexuse{\nwixident{Thm.t}}{Thm.t}{NW4ICsJQ-2aDdLu-E}\nwendcode{}\nwbegindocs{104}\nwdocspar
\begin{node}
Equality with zero is decidable since $P[x]:=0=x$ corresponds to
the function $f = D\circ(\mathcal{P}(\id, K_{0}, K_{0}, K_{1}))$. Then we have 
$\forall x\ldotp f(x)=0\iff P[x]$, which then implies $\forall x\ldotp x=0\lor x\neq0$.

The problem is that we need an additional axiom asserting
$\forall x\ldotp 0\neq x\implies\bigl(\exists y,z\ldotp x=(y,z)\bigr)$.
The converse is obviously true.
\end{node}

\begin{node}
More generally, if we want to assert the decidability of equality,
then it requires a bit of leg-work. We would first define the
predicate $P[x]:=(\pi_{1}x=\pi_{2}x)$ which corresponds to the
primitive recursive function $f(x)=D(\pi_{1}x,\pi_{2}x,0,1)$,
and then define the predicate $Q[x,y]:=P[(x,y)]$. We want to prove
that $P[x]$ is a decidable predicate (which is obvious), this will
imply the result because $\forall x\ldotp P_{0}[x]\implies\forall x,y\ldotp P_{0}[(x,y)]$ for
any unary predicate $P_{0}$ of individual-sorted terms. 
\end{node}
